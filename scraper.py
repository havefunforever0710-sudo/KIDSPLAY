...
def extract_event_details(url):
    ...
    [
      {{
        ...
        "activity_date": "請照抄網頁上的「活動時間」原文（例如：115/06/13 ~ 115/07/04），絕對不要自行發明或抓取旁邊不相干的年度預設日期！",
        "start_date": "活動起始日期 (YYYY-MM-DD)。若網頁顯示民國年(如115年)，請加1911換算成西元(2026-MM-DD)。若無確切起始日請填 null。",
        "end_date": "活動結束日期 (YYYY-MM-DD)。同樣請注意民國年換算。若為單日活動請填與起始日相同。若無確切結束日請預估至當月底或填 null。",
        "recurring_days": ["若為週期活動(如每週三、日)，請以陣列填入對應的星期數字(0代表週日,1代表週一...6代表週六)。若非週期活動請填空陣列 []"],
